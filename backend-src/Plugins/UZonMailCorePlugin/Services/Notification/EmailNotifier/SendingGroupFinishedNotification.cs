...
namespace UZonMail.Core.Services.Notification.EmailNotification
{
    /// <summary>
    /// 发件组完成通知
    /// </summary>
    ...
    public class SendingGroupFinishedNotification(SqlContext db, AppSettingService systemSetting, UserSettingService userSetting,AppSettingsManager settingsManager) : IScopedService
    {
        ...
        public async Task Notify(SendingGroup sendingGroup)
        {
            ...
            var htmlBody = template.StringValue
                .Replace("{{id}}", sendingGroup.Id.ToString())
                ...
                .Replace("{{startDate}}", sendingGroup.SendStartDate.ToString("yyyy-MM-dd HH:mm:ss"))
                .Replace("{{spentHours}}", $"{spentDate.Hours} 时 {spentDate.Minutes} 分 {spentDate} 秒");

            await new FluentEmail().WithFrom(settings.Email, "UZonMail")
                .WithTo(email)

...
export interface ITodoMailBranch { id: number, sourceConversationId: number, branchSubject: string, sourceMessageIds: number[], messages: IMailMessage[] }
...
export function createMailTodoTask (request: IUpsertTodoTask & { sourceConversationId: number, sourceMessageIds: number[], branchSubject?: string }) { return httpClient.post<ITodoTask>('/todo-tasks/mail', { data: request }) }

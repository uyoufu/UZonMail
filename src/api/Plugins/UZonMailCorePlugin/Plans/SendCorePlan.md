...
- 修复职责链无条件执行、失败后循环清理、重复缓存、发件箱跨用户冲突、游离任务、递归重试以及显式代理失败后静默直连等问题。
...
| Activation | 恢复数据库状态并注册组、发件箱和 Reader | `ISendLeaseStore` |
| Reader | Keyset 分页读取轻量描述符，按需加载完整载荷 | `ISendItemPageSource`、`ISendPayloadReader` |
| Scheduler | 公平配额、发件箱串行、重试和租约分配 | `ISendingWorkerCoordinator`、`ISendLeaseStore` |
...
| Commit | 更新邮件、收件记录、组统计和发件箱状态 | 显式 Pipeline Stage |
| Notify | 数据库提交后发布 SignalR 通知和诊断快照 | `ISendRuntimeDiagnostics` |

核心规则：

- 运行时状态按职责封装：`SendingTasksManager` 只拥有工作任务，`UserGroupTasksPools` 只拥有组，`OutboxesManager` 只拥有发件箱，`SendItemReaderPool` 只拥有 Reader；这些类型均不再继承或暴露并发字典，锁内禁止数据库和异步调用。
- `OutboxKey` 固定为 `(UserId, OutboxId)`；Transport 会话键包含 `OutboxKey`、协议配置指纹和网络出口身份。
...
WHERE SendingGroupId = @groupId
  ...
  AND (OutBoxId > @lastOutboxId OR (OutBoxId = @lastOutboxId AND Id > @lastId))
ORDER BY OutBoxId, Id
LIMIT @pageSize
```

恢复运行中的组时额外包含 `Pending`。描述符查询只选择 `Id`、`SendingGroupId`、`OutBoxId`、`TriedCount`。完整 `SendingItem`、附件、模板和变量仅在取得执行槽后由 `ISendPayloadReader` 加载当前项；重试重新生成轻量条目，完成发送周期后立即释放完整载荷引用。
...
- 指定发件箱描述符只进入对应 Outbox 队列；共享描述符进入组共享 FIFO。指定队列优先于共享队列。
- 增加复合索引 `(SendingGroupId, Status, OutBoxId, Id)`，同时生成 SQLite 和 PostgreSQL 迁移；旧单列索引可由迁移替换。
...
- 系统、组织、用户和组配额由协调器按公平份额选择工作项；单项租约由 `ISendLeaseStore` 原子申请，同一 Outbox 通过运行标记保证同时最多一个活动发送。
- 组内保持加入顺序；延迟重试保存轻量 ID、发件箱和次数，并由可取消、可观察的到期任务重新入队，不占用发送工作槽。
...
显式配置 `ProxyId` 时，如果代理不存在、禁用、无法解析、供应商不可用或没有健康端点，本次发送必须返回代理错误，禁止静默直连。Outlook Graph 当前只支持直连，Graph 发件箱配置 `ProxyId` 属于验证错误。
...
public interface IEmailTransport
{
    ...
    Task<TransportResult> ValidateAsync(IServiceProvider provider, Outbox outbox, CancellationToken token);
}
```

- SMTP 的连接、TLS 和认证复用 `SmtpConnector`；配置指纹和网络出口身份共同形成会话键。Graph 在自身 Transport 内统一解析认证配置并拒绝代理。
- `EmailSendersManager` 按 `OutboxType` 精确选择一个 Transport；缺失或重复实现均为配置错误。
- SMTP 和 Graph 都返回统一 `TransportResult`，包含分类、协议状态、错误码、服务端消息和回执 ID，不直接修改邮件或发件箱状态。
- SMTP 会话池按 `(OutboxKey, ProfileFingerprint, RouteIdentity)` 缓存，配置或出口变化自动失效；同一会话串行使用。
- Graph 会话按 `(OutboxKey, ProfileFingerprint)` 隔离，不再按邮箱字符串共享可变认证状态。
- Graph MIME 请求改为流式构造，避免 `MemoryStream -> ToArray -> Base64 string` 的多份大对象拷贝。

### 6.2 发件箱验证复用

- `OutboxValidateService` 与真实发送通过同一个 `EmailSendersManager -> IEmailTransport` 选择路径；SMTP 共同复用 `NetworkRouteResolver -> SmtpConnector`，Graph 共同复用客户端工厂与认证流程。
- SMTP 验证复用相同连接、TLS 和认证逻辑；Graph 验证复用相同 token 获取和 API 客户端构造逻辑。
- Graph 刷新凭据时按 `OutboxId` 持久化，禁止按 `(UserId, Email)` 模糊更新。
...
| 分类 | 识别条件 | 处理结果 |
| --- | --- | --- |
| 收件人永久错误 | SMTP `RecipientNotAccepted` 5xx | 当前邮件失败，发件箱继续工作 |
| 邮件永久错误 | `MessageNotAccepted` 5xx、内容或策略拒绝 | 当前邮件失败，发件箱继续工作 |
| 发件箱永久错误 | `SenderNotAccepted` 5xx；432、530、534、535、538；认证、TLS、确定配置错误 | 持久禁用发件箱并退出运行池 |
| SMTP 瞬时错误 | 421、450、451、452、454 或明确 4xx | 短重试后进入队列退避 |
| 网络或代理错误 | Socket、IO、超时、连接中断、代理端点错误 | 重建会话并轮换代理，之后队列退避 |
| 本地数据错误 | 缺收件人、正文、模板、发送器或渲染失败 | 当前邮件失败，不影响发件箱 |
| 取消 | 主机停止、组暂停或取消令牌 | 撤销租约，不增加失败或重试次数 |
| 未知错误 | 无法可靠映射 | 重试耗尽后邮件失败，发件箱仅本次运行隔离 |
...
- 已落地 `IEmailTransport`，SMTP/Graph 的发送与验证复用各自连接或认证路径；SMTP 和 Graph 会话均使用发件箱与配置指纹隔离。
...
4. 实现统一 Transport、SMTP/Graph 会话、验证复用和 Graph 凭据按 OutboxId 更新。
5. 实现 Payload、Outcome、Commit、Notify 周期及发件箱失效事务协调。
...
- 调度：验证系统硬上限、组织/用户/组公平份额、空闲借用、无抢占和发件箱单会话。
- 代理：验证优先级、准确供应商匹配、零/多匹配错误、动态刷新、健康冷却、轮换及禁止静默直连。
- Transport：数据驱动覆盖 SMTP 状态矩阵、Graph 错误、认证/TLS、网络、取消和未知异常；验证发送与发件箱校验走相同会话路径。
- 后处理：当前由状态容器与失败分类单元测试覆盖；成功、永久发件箱失败、共享改派、指定清理、无可用发件箱暂停和组完成仍应作为后续 SQLite 集成回归集持续补强。
- 契约：验证 HTTP、Quartz、SignalR、计划发送、重发、暂停、取消和 Pro 资源 JSON 不变。
- 性能：大组读取时每个工作槽只常驻当前完整实体，描述符数量受页大小、预取水位和活动组数约束。
- 数据库：SQLite/PostgreSQL 模型快照包含 `(SendingGroupId, Status, OutBoxId, Id)` 复合索引。

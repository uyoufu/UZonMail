    ...
    public class AiPrompts : IAppOptions
    {
        ...
        public string EmailBodyGeneration { get; set; } =
            """
                ...
                - Support variable replacement using the format {{uzonData.FieldName}} or {{uzonData.FieldName.SubField}} or {{any javascript function body}} (e.g., {{uzonData.Subject}}, {{uzonData.SenderAccount.Name}}, {{const a=1;return a+1;}}). Available fields under uzonData include: ["Source", "Data", "Subject", "SenderEmail", "RecipientEmail", "Body", "DateNow", "SenderAccount", "RecipientContact", "Recipients", "CC", "BCC"]. SenderAccount and RecipientContact are objects with {Name: string, Email: string} format. Use these variables dynamically in the HTML content where appropriate (e.g., in subject lines, body text, or placeholders).
        ...
        public string SubjectsSummary { get; set; } =
            """
                ...
                - Generate 5-10 subject line options, each concise (under 50 characters where possible), relevant to the content, and optimized for recipientContact visibility.
                - Incorporate variables dynamically where appropriate using the format {{uzonData.FieldName}} or {{uzonData.FieldName.SubField}} (e.g., {{uzonData.Subject}}, {{uzonData.SenderAccount.Name}}). Available fields under uzonData include: ["Source", "Data", "Subject", "SenderEmail", "RecipientEmail", "Body", "DateNow", "SenderAccount", "RecipientContact", "Recipients", "CC", "BCC"]. SenderAccount and RecipientContact are objects with {Name: string, Email: string} format.

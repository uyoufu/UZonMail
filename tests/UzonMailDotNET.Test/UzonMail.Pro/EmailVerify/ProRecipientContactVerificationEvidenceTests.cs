...
/// </summary>
[TestClass]
public sealed class ProRecipientContactVerificationEvidenceTests
{
    /// <summary>
    /// 已存在的有效快照应由不同用户共同复用
    /// </summary>
    ...
    public async Task VerifyAsync_UsesOneFreshSnapshotForDifferentUsers()
    {
        ...
        db.RecipientContactVerificationSnapshots.Add(
            new RecipientContactVerificationSnapshot
            {
                NormalizedEmail = "shared@example.com",
                State = RecipientContactVerificationState.Valid,
        ...
        var verifier = new ProRecipientContactVerificationEvidence(
            db,
            Options.Create(new RecipientContactVerificationOptions())
        );

        var firstReport = await verifier.VerifyAsync(
            new RecipientContactVerificationRequest(1, 101, "Shared@Example.com")
        );
        var secondReport = await verifier.VerifyAsync(
            new RecipientContactVerificationRequest(2, 202, "shared@example.com")
        );

        Assert.AreEqual(RecipientContactVerificationState.Valid, firstReport.State);
        Assert.AreEqual(RecipientContactVerificationState.Valid, secondReport.State);
        Assert.AreEqual(1, await db.RecipientContactVerificationSnapshots.CountAsync());
    ...
    public void SnapshotModel_UsesNormalizedEmailAsTheOnlyUniqueKey()
    {
        ...
        var entityType = db.Model.FindEntityType(typeof(RecipientContactVerificationSnapshot));
        Assert.IsNotNull(entityType);
        Assert.IsNull(entityType.FindProperty("UserId"));

        var uniqueIndex = entityType.GetIndexes().Single(x => x.IsUnique);
        CollectionAssert.AreEqual(
            new[] { nameof(RecipientContactVerificationSnapshot.NormalizedEmail) },
    ...
    public void SmtpProbe_MailFromSpfFailure_IsUnknown()
    {
        ...
        Assert.AreEqual(RecipientContactVerificationState.Unknown, evidence.State);
    ...
    public void SmtpProbe_RecipientPermanentRejection_IsInvalid()
    {
        ...
        Assert.AreEqual(RecipientContactVerificationState.Invalid, evidence.State);
    ...
    {
        var snapshot = new RecipientContactVerificationSnapshot
        {
            State = RecipientContactVerificationState.Invalid,
    ...
    public void SnapshotPolicy_UnknownStateUsesShortLifetime()
    {
        ...
        var options = new RecipientContactVerificationOptions
        {
            UnknownSnapshotLifetime = TimeSpan.FromHours(12),
            LongLivedDomainSnapshotLifetime = TimeSpan.FromDays(365),
        };

        var expiresAtUtc = RecipientContactVerificationSnapshotPolicy.GetExpiresAtUtc(
            "qq.com",

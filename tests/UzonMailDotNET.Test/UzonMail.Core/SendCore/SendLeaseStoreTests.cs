...
public sealed class SendLeaseStoreTests
{
    ...
    public void Lease_IsUniqueUntilCompleted()
    {
        ...
        var outbox = new OutboxKey(20, 30);

        Assert.IsTrue(store.TryAcquire(item, outbox, now, TimeSpan.FromMinutes(1), out var lease));
        Assert.IsFalse(store.TryAcquire(item, outbox, now, TimeSpan.FromMinutes(1), out _));
        ...
        Assert.IsTrue(
            store.TryAcquire(item, outbox, now.AddSeconds(2), TimeSpan.FromMinutes(1), out _)
        );
    }
    ...
    public void ExpiredLease_RejectsLateCompletionAndCanBeReacquired()
    {
        ...
        Assert.IsTrue(store.TryAcquire(item, outbox, now, TimeSpan.FromSeconds(5), out var lease));
        ...
        Assert.IsTrue(
            store.TryAcquire(item, outbox, now.AddSeconds(6), TimeSpan.FromSeconds(5), out _)
        );
    }
}

  ...
  >
    <div class="row justify-start items-center q-pa-md">
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.maxSendCountPerEmailDay"
        :debounce="500"
        type="number"
        label="单个发件箱每日最大发件量"
      ...
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.maxSendingBatchSize"
        :debounce="500"
        type="number"
        label="合并发件最大数量"
        placeholder="为 0 时表示不合并"
      >
        <AsyncTooltip
          :tooltip="[
            '设置多个收件人合并在一起的发件数量',
      ...
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.minOutboxCooldownSecond"
        type="number"
        :debounce="500"
        label="单个发件箱最小发件间隔 (单位: 秒)"
        placeholder="为 0 时表示不限制"
      >
      </q-input>
      ...
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.maxOutboxCooldownSecond"
        type="number"
        :debounce="500"
        label="单个发件箱最大发件间隔 (单位: 秒)"
        placeholder="为 0 时表示不限制"
      >
      </q-input>
      ...
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.minInboxCooldownHours"
        type="number"
        :debounce="500"
        label="最短收件间隔 (单位: h)"
        placeholder="为 0 时表示不限制"
      ...
      <q-input
        ...
        standout
        dense
        v-model="outboxSettingRef.replyToEmails"
        :debounce="500"
        label="回信收件人"
        placeholder="收件箱回信后的收信邮箱,若有多个使用逗号分隔"
      >
        <AsyncTooltip :tooltip="['设置回信时收信人地址', '为空时表示不设置', '有多个收信地址时,使用英文逗号分隔']" />
      </q-input>

      <div class="row justify-start items-center col-auto-4">
        <q-checkbox
          dense
          v-model="outboxSettingRef.enableEmailTracker"
          label="启用邮件跟踪"
          color="secondary"
          class="q-ml-sm"
          keep-color
...
<script lang="ts" setup>
  ...
  watch(
    () => outboxSettingRef.value.enableEmailTracker,
    async (newValue) => {
      if (!newValue) return

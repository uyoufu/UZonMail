<template>
  <q-expansion-item v-model="expanded" popup :icon="icon" :label="label" :caption="caption"
    ...
    <div class="row justify-start items-center q-pa-md">
      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.maxSendCountPerEmailDay"
        :debounce="500" type="number" label="单个发件箱每日最大发件量" placeholder="为 0 时表示不限制">
        <AsyncTooltip :tooltip="['设置发件箱单日最大发件量', '为 0 表示不限制']" />
      </q-input>

      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.maxSendingBatchSize" :debounce="500"
      ...
      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.minOutboxCooldownSecond"
        type="number" :debounce="500" label="单个发件箱最小发件间隔 (单位: 秒)" placeholder="为 0 时表示不限制">
      </q-input>

      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.maxOutboxCooldownSecond"
        type="number" :debounce="500" label="单个发件箱最大发件间隔 (单位: 秒)" placeholder="为 0 时表示不限制">
      </q-input>

      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.minInboxCooldownHours" type="number"
      ...
      <q-input outlined class="col-auto-4" standout dense v-model="outboxSettingRef.changeIpAfterEmailCount"
        :debounce="500" type="number" label="单代理每个发件箱发件数" placeholder="为 0 时表示不限制">
        <AsyncTooltip :tooltip="['某个邮箱使用某个代理的发件总数', '小于等于 0 时表示不限制']" />
      </q-input>
    </div>
  </q-expansion-item>
</template>
